...
export function useTable(tableName, fallbackKey, orderColumn = "order_index") {
  ...
  onMounted(async () => {
    ...
    if (error) {
      console.error(
        `[supabase] erro ao buscar "${tableName}":`,
        error.message,
        error,
      );
    } else if (!rows || rows.length === 0) {
      console.warn(
        `[supabase] tabela "${tableName}" retornou vazia — usando fallback.`,
      );
    } else {
      data.value = rows;
      console.info(
        `[supabase] fonte: supabase — "${tableName}" (${rows.length} linhas)`,
      );
    }
    loading.value = false;
  });
...
export function useProfile() {
  ...
  onMounted(async () => {
    if (!supabase) {
      console.warn(
        '[supabase] cliente não configurado — usando fallback para "profile".',
      );
      loading.value = false;
      return;
    }
    ...
    if (error) {
      console.error(
        '[supabase] erro ao buscar "profile":',
        error.message,
        error,
      );
    } else if (!rows || rows.length === 0) {
      console.warn(
        '[supabase] tabela "profile" retornou vazia — usando fallback.',
      );
    } else {
      data.value = rows[0];
